...
class CostControlManager:
    ...
    def estimate_api_cost(
        ...
        self.logger.debug(
            f"Cost estimate: {tokens_input}+{tokens_output} tokens = ${total_cost:.6f}"
        )
    ...
    def pre_api_check(
        ...
        if not allowed:
            ...
            self.logger.error(
                f"Session allowed: {session_allowed}, Daily: {daily_allowed}, Monthly: {monthly_allowed}"
            )
    ...
    def _get_session_cost(self, session_id: str) -> float:
        ...
        with contextlib.closing(sqlite3.connect(self.db_path)) as conn:
            cursor = conn.execute(
                "SELECT SUM(cost_usd) FROM api_costs WHERE session_id = ?", (session_id,)
            )
    ...
    def _get_daily_cost(self, date_str: str) -> float:
        ...
        with contextlib.closing(sqlite3.connect(self.db_path)) as conn:
            cursor = conn.execute(
                "SELECT SUM(cost_usd) FROM api_costs WHERE DATE(timestamp) = ?", (date_str,)
            )
            result = cursor.fetchone()[0]
            return result if result else 0.0
    ...
    def _generate_alerts(
        ...
        daily_approaching = (
            daily_tracking.daily_budget_status == DailyBudgetStatus.APPROACHING_LIMIT
        )
        monthly_approaching = monthly_tracking.monthly_budget_status in [
            MonthlyBudgetStatus.WARNING,
            MonthlyBudgetStatus.CRITICAL,
        ]
        ...
        return CostAlerts(
            ...
            last_alert_timestamp=(
                datetime.now().isoformat()
                if any([session_over, daily_approaching, monthly_approaching, unusual_spike])
                else None
            ),
        )
    ...
    def _log_alert(self, alert_type: str, severity: str, message: str) -> None:
...
if __name__ == "__main__":
    ...
    allowed, check_result = cost_manager.pre_api_check(
        session_id=session_id, tokens_input=1000, tokens_output=500, model="gpt-4o-2024-11-20"
    )
