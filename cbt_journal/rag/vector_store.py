...
class CBTVectorStore:
    ...
    def store_session(
        self, session_data: Dict[str, Any], embedding: List[float], session_id: Optional[str] = None
    ) -> str:
        """
        ...
        if len(embedding) != 3072:
            raise CBTVectorStoreError(f"Embedding dimension {len(embedding)}, expected 3072")
        ...
        # Create point
        point = PointStruct(id=session_id, vector=embedding, payload=session_data)
            ...
            for key, value in filters.items():
                if isinstance(value, str):
                    ...
                    filter_conditions.append(
                        FieldCondition(
                            key=key,
                            range=Range(gte=range_filter.get("gte"), lte=range_filter.get("lte")),
                        )
                    )
    ...
    def update_session(
        self, session_id: str, updates: Dict[str, Any], new_embedding: Optional[List[float]] = None
    ) -> bool:
        """
        ...
        try:
            ...
            if new_embedding is not None:
                ...
                point = PointStruct(id=session_id, vector=new_embedding, payload=updated_payload)
            else:
                ...
                if not existing_points:
                    raise CBTVectorStoreError(
                        f"Session {session_id} not found for vector retrieval"
                    )
    ...
    def delete_session(self, session_id: str) -> bool:
        ...
        try:
            result = self.client.delete(
                collection_name=self.collection_name, points_selector=[session_id]
            )
    ...
    def search_by_filters(self, filters: Dict[str, Any], limit: int = 50) -> List[Dict[str, Any]]:
        """
...
def create_vector_store(
    ...
    return CBTVectorStore(
        qdrant_client=qdrant_client, cost_manager=cost_manager, collection_name=collection_name
    )
